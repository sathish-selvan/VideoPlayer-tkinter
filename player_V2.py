        ...
        try:
            ...
            if not player.is_playing():
                ...
                try:
                    ...
                    print(video_name)
                    nan = " "
                    l1 = list(dataframe[video_name].dropna())
                ...
                coordinates = []
                global step_size
                step_size  = round((SLIDER_SIZE)/duration,3)
                print(step_size,duration)
                for i in length:
                    x = find_coordinates(i,step_size)
                    coordinates.append(x)
                print(coordinates)
                global label
                label = {}
                for point in coordinates:
                    def set_video(a=point):
                        global step_size
                        print(a,int(a/step_size))
                        timeSlider.set(int(a/step_size))
                    label[point] = Button(root, text="@",command=set_video)
                    label[point].place(x=point+OFFSET, y = 600)
